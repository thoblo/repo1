...
### Energy, Valence etc door de jaren heen
...
### Boxplots Energy, Valence etc voor elk koor
...
Alle_koren_juice <- 
  recipe(track.name ~
           ...
           c01 + c02 + c03 + c04 + c05 + c06 +
           c07 + c08 + c09 + c10 + c11 + c12,
         data = Alle_koren_liedjes) %>% 
  step_range(all_predictors()) %>% 
  prep(Alle_koren_liedjes %>% mutate(track.name = str_trunc(track.name, 20))) %>% 
  juice %>% 
  column_to_rownames('track.name')
